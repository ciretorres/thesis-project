...
const spriteMap = new TextureLoader().load("../public/favicon.ico");
const spriteMaterial = new SpriteMaterial({ map: spriteMap });
for (let dec = -90; dec < 90; dec += 20) {
  for (let ra = 0; ra <= 360; ra++) {
    const sprite = new Sprite(spriteMaterial);
    const x =
      radio * Math.cos((dec * Math.PI) / 180) * Math.cos((ra * Math.PI) / 180);
    const y = radio * Math.sin((dec * Math.PI) / 180);
    const z =
      radio * Math.cos((dec * Math.PI) / 180) * Math.sin((ra * Math.PI) / 180);
...
//    * @see https://threejs.org/docs/#api/en/scenes/Scene
//    */
//   const createScene = (backgroundColor = new THREE.Color(0x444444)) => {
